  ...
  const handleDeleteNoti = (notiID) => {
    ...
    fetch("http://" + IP + ":5000/noti", {
      ...
      .then((data) => {
        console.log(data);
        if (data[0] === "Sucess") {
          setListOfNoti(Array.from(data[1]));
          setListOfNotiSFX(CreateListOfNoti(Array.from(data[1])));
        }
      })
      .catch((error) => {
        setServiceDown(true);
  ...
  return (
    <div>
      <Flex
        ...
        justifyContent="left"
        background={
          isUnauthorizedAccess || isServiceDown ? "white" : "green.500"
        }
      >
        {errorMessage === ""
          ...
          >
            ...
            >
              <Stack direction="column" spacing={5}>
                <Button backgroundColor="green.100" rounded={20}>
                  PATIENT
                </Button>
                <Button backgroundColor="green.100" rounded={20}>
                  PHYSICIAN
